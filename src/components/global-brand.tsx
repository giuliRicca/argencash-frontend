...
const sidebarBrandPathPrefixes = ["/dashboard", "/settings", "/accounts", "/assistant"];
const hiddenBrandPaths = new Set(["/", "/login", "/register", "/unauthorized"]);
...
export function GlobalBrand() {
  ...
  const isSidebarBrandPath = sidebarBrandPathPrefixes.some((prefix) => pathname === prefix || pathname.startsWith(`${prefix}/`));

  if (hiddenBrandPaths.has(pathname) || isSidebarBrandPath || (isSecuredPath && !accessToken)) {
    return null;
  }
